...
##################################################################################
def advancd_numpy_amount_emiss(str,tran_count,Beg,END,lastcol):
    content = list(open(str))
    tot_col = eval(content[0])
    coun_1 = 0
    coun_2 = 0
    ...
    for apic in range(1,(len(content)-tot_col)):
        ...
        if eval(content[apic+tot_col].split()[-1]) == 1:
            coun_1 += 1
        if eval(content[apic+tot_col].split()[-1]) == 2:
            coun_2 += 1
    return total_numpy,coun_1,coun_2
def advancd_emission_pro(str,tran_count,Beg,END,lastcol):
    ...
    num_numpy,n1,n2= advancd_numpy_amount_emiss(Symbol_File,tran_count,Beg,END,lastcol)
...
############################################################################
# Bonus
def Bonus(State_File, Symbol_File, Query_File): 
